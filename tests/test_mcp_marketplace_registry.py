...
def test_fetch_registry_catalog_defaults_to_bounded_page_count():
    ...
    entries = fetch_registry_catalog("https://registry.example/v0.1/servers", source_id="official-mcp-registry", source_priority=60, client=client)
            ...
            return FakeRegistryResponse({
                "servers": [{
                    "server": {
                        ...
                        "description": "One",
                        "version": "1",
                        "packages": [{"registryType": "npm", "identifier": "one-mcp"}],
                    },
                    "_meta": {"io.modelcontextprotocol.registry/official": {"status": "active", "isLatest": True}},
                }],
                "metadata": {"nextCursor": "slow-page", "count": 1},
            })
        raise TimeoutError("slow registry page")


def test_fetch_registry_catalog_keeps_completed_pages_when_later_page_fails():
    entries = fetch_registry_catalog(
    ...
    assert [entry["id"] for entry in entries] == ["registry-one-npm-one-mcp"]
