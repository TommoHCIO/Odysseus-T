...
read/write files, create and edit documents, generate images, manage Obsidian knowledge, and more. \
...
- User identity facts/preferences ("my name is <name>", "I live in <place>", "I prefer concise replies", "call me <name>") → use `manage_knowledge` with action=add. NEVER use `manage_contact` for facts about the user unless the user explicitly says to create/update a contact and provides contact details such as an email or phone.
- "Create/add/write a note" / "notes" / "todos" / "remind me to X at <time>" → use `manage_notes`. Do NOT store notes in `manage_knowledge`; Obsidian knowledge is for persistent facts/preferences/project context about the user, not note content. For reminders, include a `due_date`; for todos, use `note_type=checklist` when appropriate.
...
- "Open/show <panel>" (documents, library, gallery, email, inbox, sessions, Obsidian/knowledge/memory, skills, settings, notes, cookbook) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, images→gallery, mail/inbox/emails→email, chats/history→sessions, memory/memories/brain/knowledge/skills→obsidian, preferences→settings, models/serve/serving→cookbook. CRITICAL: "open memory/memories/brain/knowledge" / "open skills" / "open notes" / "open documents" / "open cookbook" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool. The "manage_*" tools list contents in chat; `ui_control open_panel` opens the visual modal the user is asking for.
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_knowledge": "- ```manage_knowledge``` — Preferred Obsidian-backed durable knowledge tool. Manage persistent facts, identity, preferences, project context, requirements, goals, todos, and references that should survive across chats. Line 1 = action (list/add/edit/delete/search), rest = content. Use when user says 'remember this', states identity facts like 'my name is <name>' / 'call me <name>' / 'I live in <place>', or asks about stored knowledge.",
    "manage_memory": "- ```manage_memory``` — Deprecated compatibility alias for ```manage_knowledge```. It reads and writes the same Obsidian knowledge notes.",
...
Bulk delete/archive/mark emails. Use this for "delete all those" after listing emails. Pass the exact UIDs and the same account from the list result, then report only the tool result.""",
    ...
    "manage_contact": "- ```manage_contact``` — Create/update/delete/list CardDAV contacts. Args (JSON): {\"action\": \"list|add|update|delete\", \"name\": \"...\", \"email\": \"...\", \"uid\": \"...\"}. Use only for explicit address-book/contact requests with contact details. Do NOT use for user identity facts like 'my name is <name>'; save those with manage_knowledge. For update/delete, call action=list first to get the uid.",
    "manage_calendar": """\
```manage_calendar
...
def _build_system_prompt(
    ...
    try:
        ...
        if last_user and _skills_on:
            ...
            # Obsidian Skills settings → "Auto-approve skills" toggle +
            ...
            #    manage_knowledge(action='delete_all')"
...
async def stream_agent_loop(
    ...
    if not _relevant_tools and _retrieval_query:
        ...
        _relevant_tools.update({"create_document", "manage_knowledge", "manage_memory", "manage_notes"})
